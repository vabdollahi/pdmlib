...
def create_default_price_provider(
    location: GeospatialLocation,
    start_date_time: datetime,
    end_date_time: datetime,
    storage: Optional[DataStorage] = None,
    ...
    """
    Create a default CSV price provider for fallback scenarios.

    Args:
        location: Geographic location (not used for CSV provider)
        start_date_time: Start datetime for data range
        end_date_time: End datetime for data range
        storage: Optional data storage configuration (not used for CSV provider)

    Returns:
        Default CSV price provider instance
    """
    logger.info("Creating default CSV price provider")
    from pathlib import Path

    # Use the sample price data for testing
    default_csv_path = (
    ...
    config = CSVPriceProviderConfig(data=str(default_csv_path))
    return create_price_provider_from_config(
        config, location, start_date_time, end_date_time, storage
    )


def create_default_weather_provider(
    location: GeospatialLocation,
    start_date_time: datetime,
    end_date_time: datetime,
    storage: Optional[DataStorage] = None,
    ...
    """
    Create a default CSV weather provider for fallback scenarios.
    ...
    Returns:
        Default CSV weather provider instance
    """
    logger.info("Creating default CSV weather provider")
    from pathlib import Path

    # Use the sample weather data for testing
    default_csv_path = (
        Path(__file__).parent.parent.parent.parent
        / "tests"
        / "data"
    ...
    config = CSVWeatherProviderConfig(data=str(default_csv_path))
