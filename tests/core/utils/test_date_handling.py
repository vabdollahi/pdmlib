...
class TestParseDatetimeInput:
    ...
    def test_parse_date_string(self):
        ...
        expected = datetime(2025, 1, 1, 0, 0, 0, tzinfo=pd.Timestamp.utcnow().tz)
    ...
    def test_parse_datetime_string_with_seconds(self):
        ...
        result = parse_datetime_input("2025-01-01 14:30:45")
        expected = datetime(2025, 1, 1, 14, 30, 45, tzinfo=pd.Timestamp.utcnow().tz)
        assert result.replace(tzinfo=None) == expected.replace(tzinfo=None)

    def test_parse_datetime_string_without_seconds(self):
        """Test parsing datetime string without seconds."""
        result = parse_datetime_input("2025-01-01 14:30")
        expected = datetime(2025, 1, 1, 14, 30, 0, tzinfo=pd.Timestamp.utcnow().tz)
        assert result.replace(tzinfo=None) == expected.replace(tzinfo=None)
    ...
    def test_parse_datetime_object(self):
        ...
        input_dt = datetime(2025, 1, 1, 14, 30, 45)
    ...
    def test_parse_with_custom_default_time(self):
        ...
        result = parse_datetime_input("2025-01-01", default_time=custom_time)
        expected = datetime(2025, 1, 1, 12, 30, 0, tzinfo=pd.Timestamp.utcnow().tz)
        assert result.replace(tzinfo=None) == expected.replace(tzinfo=None)
...
class TestNormalizeDateRange:
    ...
    def test_normalize_datetime_strings(self):
        ...
        start, end = normalize_date_range("2025-01-01 10:30", "2025-01-01 16:45")
    ...
    def test_normalize_mixed_inputs(self):
        ...
        start, end = normalize_date_range("2025-01-01", "2025-01-01 12:00")
    ...
    def test_normalize_invalid_range_raises_error(self):
        ...
        with pytest.raises(
            ...
            normalize_date_range("2025-01-02", "2025-01-01")
...
class TestSplitDatetimeRangeByDays:
    ...
    def test_split_multi_day_range(self):
        ...
        start = datetime(2025, 1, 1, 14, 30)
        end = datetime(2025, 1, 3, 10, 15)
        ...
        assert chunks[0][0] == datetime(2025, 1, 1, 14, 30)
        ...
        assert chunks[2][1] == datetime(2025, 1, 3, 10, 15)
...
class TestBackwardCompatibility:
    ...
    def test_normalize_matches_current_behavior(self):
        ...
        start, end = normalize_date_range("2025-01-01", "2025-01-10")

        # Should start at beginning of first day
        assert start == datetime(2025, 1, 1, 0, 0, 0, tzinfo=start.tzinfo)
...
class TestRoundDatetimeToInterval:
    ...
    def test_round_to_five_minutes_down(self):
        ...
        expected = datetime(2025, 1, 1, 10, 0, 0)
    ...
    def test_round_to_fifteen_minutes_up(self):
        ...
        expected = datetime(2025, 1, 1, 10, 15, 0)
        assert result.replace(tzinfo=None) == expected
    ...
    def test_round_to_hourly_up(self):
        ...
        expected = datetime(2025, 1, 1, 11, 0, 0)
        assert result.replace(tzinfo=None) == expected
    ...
    def test_round_to_daily_up(self):
        ...
        expected = datetime(2025, 1, 2, 0, 0, 0)
        assert result.replace(tzinfo=None) == expected
    ...
    def test_round_preserves_timezone(self):
        ...
        dt = datetime(2025, 1, 1, 10, 1, 23, tzinfo=tz)
        result = round_datetime_to_interval(dt, TimeInterval.FIVE_MINUTES, "down")
...
class TestRoundDatetimeRangeToInterval:
    ...
    def test_round_range_five_minutes(self):
        ...
        assert rounded_start.replace(tzinfo=None) == datetime(2025, 1, 1, 10, 0, 0)
        assert rounded_end.replace(tzinfo=None) == datetime(2025, 1, 1, 14, 50, 0)
    ...
    def test_round_range_hourly(self):
        ...
        assert rounded_start.replace(tzinfo=None) == datetime(2025, 1, 1, 10, 0, 0)
        assert rounded_end.replace(tzinfo=None) == datetime(2025, 1, 1, 15, 0, 0)
    ...
    def test_round_range_daily(self):
        ...
        assert rounded_start.replace(tzinfo=None) == datetime(2025, 1, 1, 0, 0, 0)
        assert rounded_end.replace(tzinfo=None) == datetime(2025, 1, 2, 0, 0, 0)
    ...
    def test_round_range_expands_correctly(self):
        ...
        start = datetime(2025, 1, 1, 10, 1, 0)
        ...
        assert rounded_start.replace(tzinfo=None) == datetime(2025, 1, 1, 10, 0, 0)
        assert rounded_end.replace(tzinfo=None) == datetime(2025, 1, 1, 10, 5, 0)
...
class TestNormalizeDateRangeWithInterval:
    ...
    def test_normalize_with_hourly_interval(self):
        ...
        start, end = normalize_date_range(
            "2025-01-01 10:01:23", "2025-01-01 14:47:15", interval=TimeInterval.HOURLY
        )
    ...
    def test_normalize_with_daily_interval(self):
        ...
        start, end = normalize_date_range(
            "2025-01-01 10:01:23", "2025-01-01 14:47:15", interval=TimeInterval.DAILY
        )

        assert start.hour == 0 and start.minute == 0 and start.second == 0
        assert end.hour == 0 and end.minute == 0 and end.second == 0
        assert end.date() == datetime(2025, 1, 2).date()
    ...
    def test_normalize_without_interval_unchanged(self):
        ...
        start_with_none, end_with_none = normalize_date_range(
            "2025-01-01 10:01:23", "2025-01-01 14:47:15", interval=None
        )
    ...
    def test_normalize_date_strings_with_interval(self):
        ...
        start, end = normalize_date_range(
            "2025-01-01", "2025-01-02", interval=TimeInterval.FIVE_MINUTES
        )
        ...
        assert end.hour == 0 and end.minute == 0 and end.second == 0
        assert end.date() == datetime(2025, 1, 3).date()
...
class TestIntervalRoundingExamples:
    ...
    def test_user_example_five_minutes(self):
        """Test the exact example from user: 2025-01-01 10:01:23 with 5min interval."""
        start_input = "2025-01-01 10:01:23"
        interval = TimeInterval.FIVE_MINUTES

        start, _ = normalize_date_range(start_input, start_input, interval=interval)

        # Should be rounded down to 2025-01-01 10:00:00
        expected = datetime(2025, 1, 1, 10, 0, 0)
    ...
    def test_various_minute_boundaries(self):
        ...
        test_cases = [
            ...
            ("2025-01-01 10:01:23", TimeInterval.FIVE_MINUTES, 0),
            ("2025-01-01 10:03:45", TimeInterval.FIVE_MINUTES, 0),
            ("2025-01-01 10:07:12", TimeInterval.FIVE_MINUTES, 5),
            ("2025-01-01 10:13:56", TimeInterval.FIVE_MINUTES, 10),
            ("2025-01-01 10:02:30", TimeInterval.FIFTEEN_MINUTES, 0),
            ("2025-01-01 10:08:45", TimeInterval.FIFTEEN_MINUTES, 0),
            ("2025-01-01 10:17:23", TimeInterval.FIFTEEN_MINUTES, 15),
            ("2025-01-01 10:31:15", TimeInterval.FIFTEEN_MINUTES, 30),
        ]
    ...
    def test_end_time_rounding_up(self):
        ...
        test_cases = [
            ...
            ("2025-01-01 10:01:23", TimeInterval.FIVE_MINUTES, 5),
            ("2025-01-01 10:03:45", TimeInterval.FIVE_MINUTES, 5),
            ("2025-01-01 10:07:12", TimeInterval.FIVE_MINUTES, 10),
            ("2025-01-01 10:02:30", TimeInterval.FIFTEEN_MINUTES, 15),
            ("2025-01-01 10:17:23", TimeInterval.FIFTEEN_MINUTES, 30),
        ]
